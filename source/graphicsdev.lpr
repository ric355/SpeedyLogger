...
begin
  ...
  gaugelist[5] := TValueGauge.Create(Width - 600, Height - 70, 150, 50, 0, 1000, 0, dtByte, 0, 30);
...
begin
  ...
  begin
    ...
    begin
      ...
      drawgauges;
      {$else}
      // this is just to stop the main loop using loads of processor time
      // as the serial reading is in a separate thread, and screen updates
      // occur on that thread via the callback. Needs some testing though.
      sleep(100);
      {$endif}
